...
void dobraTamanhoArquivo(int n)
{
    ...
    // Preenchendo a última coluna nas linhas pares (sem vizinho à direita)
    for (y = 0; y < n * 2; y += 2)
    {
        arrayBidimensional[y][n * 2 - 1] = arrayBidimensional[y][n * 2 - 2];
    }
    ...
    for (y = 1; y < n * 2 - 1; y += 2)
    {
        for (x = 1; x < n * 2 - 1; x += 2)
        {
            arrayBidimensional[y][x] = (arrayBidimensional[y - 1][x] + arrayBidimensional[y + 1][x] +
                                        arrayBidimensional[y][x - 1] + arrayBidimensional[y][x + 1]) /
                                       4;
        }
    }

    // Preenchendo a última linha (y = n*2 - 1) com o vizinho acima
    for (x = 0; x < n * 2 - 1; x++)
    {
        arrayBidimensional[n * 2 - 1][x] = arrayBidimensional[n * 2 - 2][x];
    }

    // Preenchendo o canto inferior direito com a média dos vizinhos
    arrayBidimensional[n * 2 - 1][n * 2 - 1] =
        (arrayBidimensional[n * 2 - 2][n * 2 - 1] + arrayBidimensional[n * 2 - 1][n * 2 - 2]) / 2;
...
int main()
{
    ...
    // Cria novas linhas seguindo o padrão da primeira, descartando
    // as primeiras 'extra' linhas para dissipar o padrão senoidal
    num_linhas = 1;
    if (num_linhas > extra)
    ...
    while (num_linhas < totalLinhas)
    {
        ...
        // Free previous firstArrayLine to avoid memory leak
        free(firstArrayLine);

        // Update pointers
        firstArrayLine = secondArrayLine;
        secondArrayLine = newLine;
    }
