...
int main()
{
    ...
    salvarLinha(firstArrayLine, n); // Save the first line generated by sin
    unsigned char *secondArrayLine = novaLinha(firstArrayLine, n, delta);
    salvarLinha(secondArrayLine, n); // Save the second line too

    num_linhas = 2; // We've already saved 2 lines
    while (num_linhas < n)
    { // Change to n instead of n*2
